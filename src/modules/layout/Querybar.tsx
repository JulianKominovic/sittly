...
const Querybar = () => {
  ...
  const icon = useMemo(() => {
    return INDEX.find(({ module: mod }) => mod === location.pathname.slice(1))?.icon;
  }, [location.pathname]);
  useEffect(() => {
  ...
  return (
    <header className="flex items-center h-12 pr-4">
      <div className="text-xl ml-4 rounded-lg bg-gray-200 p-1 text-neutral-900">{icon}</div>
      <input
        tabIndex={-1}
        id="query-bar"
        ref={searchbar}
        ...
        className="text-xl h-full w-full outline-none px-2 mx-4 placeholder:text-gray-600 bg-transparent"
